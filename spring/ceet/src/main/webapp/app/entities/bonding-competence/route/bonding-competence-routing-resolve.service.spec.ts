...
import bondingCompetenceResolve from './bonding-competence-routing-resolve.service';

describe('BondingCompetence routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IBondingCompetence returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          bondingCompetenceResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(bondingCompetenceResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');

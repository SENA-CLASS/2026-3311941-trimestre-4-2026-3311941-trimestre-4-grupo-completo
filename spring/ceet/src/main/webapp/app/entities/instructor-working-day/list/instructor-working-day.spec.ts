...
import { sampleWithRequiredData } from '../instructor-working-day.test-samples';
...
describe('InstructorWorkingDay Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: InstructorWorkingDay;
  let fixture: ComponentFixture<InstructorWorkingDay>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(InstructorWorkingDay);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'a6718508-6ea5-4a16-8155-bfbc48acf256' }], {
    ...
    expect(comp.instructorWorkingDays()[0]).toEqual(expect.objectContaining({ id: 'a6718508-6ea5-4a16-8155-bfbc48acf256' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'a6718508-6ea5-4a16-8155-bfbc48acf256' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.instructorWorkingDays()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'a6718508-6ea5-4a16-8155-bfbc48acf256' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to instructorWorkingDayService', () => {
      ...
      vitest.spyOn(service, 'getInstructorWorkingDayIdentifier');
      const id = comp.trackId(entity);
      expect(service.getInstructorWorkingDayIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.instructorWorkingDaysParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

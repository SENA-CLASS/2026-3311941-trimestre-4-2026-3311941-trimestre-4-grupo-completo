...
import { sampleWithRequiredData } from '../check-list-course.test-samples';
...
describe('CheckListCourse Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: CheckListCourse;
  let fixture: ComponentFixture<CheckListCourse>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CheckListCourse);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'fe0e76c2-b0cd-455f-82aa-49d4fdb3fd1e' }], {
    ...
    expect(comp.checkListCourses()[0]).toEqual(expect.objectContaining({ id: 'fe0e76c2-b0cd-455f-82aa-49d4fdb3fd1e' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'fe0e76c2-b0cd-455f-82aa-49d4fdb3fd1e' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.checkListCourses()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'fe0e76c2-b0cd-455f-82aa-49d4fdb3fd1e' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to checkListCourseService', () => {
      const entity = { id: 'fe0e76c2-b0cd-455f-82aa-49d4fdb3fd1e' };
      vitest.spyOn(service, 'getCheckListCourseIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCheckListCourseIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.checkListCoursesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

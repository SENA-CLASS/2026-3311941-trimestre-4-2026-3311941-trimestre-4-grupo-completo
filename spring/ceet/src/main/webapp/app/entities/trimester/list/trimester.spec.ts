...
import { sampleWithRequiredData } from '../trimester.test-samples';
...
describe('Trimester Management Component', () => {
  ...
  let comp: Trimester;
  let fixture: ComponentFixture<Trimester>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Trimester);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '23943bac-1112-4eec-a590-f244e6fcb60b' }], {
    ...
    expect(comp.trimesters()[0]).toEqual(expect.objectContaining({ id: '23943bac-1112-4eec-a590-f244e6fcb60b' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '23943bac-1112-4eec-a590-f244e6fcb60b' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.trimesters()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '23943bac-1112-4eec-a590-f244e6fcb60b' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to trimesterService', () => {
      ...
      vitest.spyOn(service, 'getTrimesterIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTrimesterIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.trimestersParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

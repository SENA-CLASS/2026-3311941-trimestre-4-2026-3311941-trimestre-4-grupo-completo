...
import { sampleWithRequiredData } from '../classroom-limitation.test-samples';
...
describe('ClassroomLimitation Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: ClassroomLimitation;
  let fixture: ComponentFixture<ClassroomLimitation>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(ClassroomLimitation);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'ba08a49b-1200-43e0-801b-3547f699926b' }], {
    ...
    expect(comp.classroomLimitations()[0]).toEqual(expect.objectContaining({ id: 'ba08a49b-1200-43e0-801b-3547f699926b' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'ba08a49b-1200-43e0-801b-3547f699926b' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.classroomLimitations()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'ba08a49b-1200-43e0-801b-3547f699926b' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to classroomLimitationService', () => {
      const entity = { id: 'ba08a49b-1200-43e0-801b-3547f699926b' };
      vitest.spyOn(service, 'getClassroomLimitationIdentifier');
      const id = comp.trackId(entity);
      expect(service.getClassroomLimitationIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.classroomLimitationsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

...
import classroomLimitationResolve from './classroom-limitation-routing-resolve.service';

describe('ClassroomLimitation routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IClassroomLimitation returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          classroomLimitationResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(classroomLimitationResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');

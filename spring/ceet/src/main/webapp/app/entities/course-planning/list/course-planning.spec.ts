...
import { sampleWithRequiredData } from '../course-planning.test-samples';
...
describe('CoursePlanning Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: CoursePlanning;
  let fixture: ComponentFixture<CoursePlanning>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CoursePlanning);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '4b803aa8-fa56-47c2-883b-22da200d8885' }], {
    ...
    expect(comp.coursePlannings()[0]).toEqual(expect.objectContaining({ id: '4b803aa8-fa56-47c2-883b-22da200d8885' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '4b803aa8-fa56-47c2-883b-22da200d8885' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.coursePlannings()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '4b803aa8-fa56-47c2-883b-22da200d8885' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to coursePlanningService', () => {
      const entity = { id: '4b803aa8-fa56-47c2-883b-22da200d8885' };
      vitest.spyOn(service, 'getCoursePlanningIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCoursePlanningIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.coursePlanningsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

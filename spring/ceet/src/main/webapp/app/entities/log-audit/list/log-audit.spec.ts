...
import { sampleWithRequiredData } from '../log-audit.test-samples';
...
describe('LogAudit Management Component', () => {
  ...
  let comp: LogAudit;
  let fixture: ComponentFixture<LogAudit>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(LogAudit);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '71eef2e7-cb01-4349-bad7-c9a369beb19a' }], {
    ...
    expect(comp.logAudits()[0]).toEqual(expect.objectContaining({ id: '71eef2e7-cb01-4349-bad7-c9a369beb19a' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '71eef2e7-cb01-4349-bad7-c9a369beb19a' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.logAudits()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '71eef2e7-cb01-4349-bad7-c9a369beb19a' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to logAuditService', () => {
      const entity = { id: '71eef2e7-cb01-4349-bad7-c9a369beb19a' };
      vitest.spyOn(service, 'getLogAuditIdentifier');
      const id = comp.trackId(entity);
      expect(service.getLogAuditIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.logAuditsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

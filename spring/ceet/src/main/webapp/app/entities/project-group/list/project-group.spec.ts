...
import { sampleWithRequiredData } from '../project-group.test-samples';
...
describe('ProjectGroup Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: ProjectGroup;
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '09699592-062b-40e5-aa51-6ed07e22b997' }], {
    ...
    expect(comp.projectGroups()[0]).toEqual(expect.objectContaining({ id: '09699592-062b-40e5-aa51-6ed07e22b997' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '09699592-062b-40e5-aa51-6ed07e22b997' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.projectGroups()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '09699592-062b-40e5-aa51-6ed07e22b997' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to projectGroupService', () => {
      ...
      vitest.spyOn(service, 'getProjectGroupIdentifier');
      const id = comp.trackId(entity);
      expect(service.getProjectGroupIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.projectGroupsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

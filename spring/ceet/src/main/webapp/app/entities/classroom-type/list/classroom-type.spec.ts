...
import { sampleWithRequiredData } from '../classroom-type.test-samples';
...
describe('ClassroomType Management Component', () => {
  ...
  let comp: ClassroomType;
  let fixture: ComponentFixture<ClassroomType>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(ClassroomType);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '78a499bc-a8ee-4f5e-beea-8694e07463a8' }], {
    ...
    expect(comp.classroomTypes()[0]).toEqual(expect.objectContaining({ id: '78a499bc-a8ee-4f5e-beea-8694e07463a8' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '78a499bc-a8ee-4f5e-beea-8694e07463a8' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.classroomTypes()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '78a499bc-a8ee-4f5e-beea-8694e07463a8' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to classroomTypeService', () => {
      ...
      vitest.spyOn(service, 'getClassroomTypeIdentifier');
      const id = comp.trackId(entity);
      expect(service.getClassroomTypeIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.classroomTypesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

...
import { sampleWithRequiredData } from '../working-day-course.test-samples';

import { WorkingDayCourse } from './working-day-course';

vitest.useFakeTimers();

describe('WorkingDayCourse Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: WorkingDayCourse;
  let fixture: ComponentFixture<WorkingDayCourse>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(WorkingDayCourse);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '3317990a-8fb1-4654-821f-70185269b7b0' }], {
    ...
    expect(comp.workingDayCourses()[0]).toEqual(expect.objectContaining({ id: '3317990a-8fb1-4654-821f-70185269b7b0' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '3317990a-8fb1-4654-821f-70185269b7b0' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.workingDayCourses()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '3317990a-8fb1-4654-821f-70185269b7b0' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to workingDayCourseService', () => {
      ...
      vitest.spyOn(service, 'getWorkingDayCourseIdentifier');
      const id = comp.trackId(entity);
      expect(service.getWorkingDayCourseIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.workingDayCoursesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

...
import { sampleWithRequiredData } from '../working-day.test-samples';
...
describe('WorkingDay Management Component', () => {
  ...
  let comp: WorkingDay;
  let fixture: ComponentFixture<WorkingDay>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(WorkingDay);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'a0aff8e9-f539-4c17-817e-bbf8dfab13f4' }], {
    ...
    expect(comp.workingDays()[0]).toEqual(expect.objectContaining({ id: 'a0aff8e9-f539-4c17-817e-bbf8dfab13f4' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'a0aff8e9-f539-4c17-817e-bbf8dfab13f4' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.workingDays()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'a0aff8e9-f539-4c17-817e-bbf8dfab13f4' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to workingDayService', () => {
      const entity = { id: 'a0aff8e9-f539-4c17-817e-bbf8dfab13f4' };
      vitest.spyOn(service, 'getWorkingDayIdentifier');
      const id = comp.trackId(entity);
      expect(service.getWorkingDayIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.workingDaysParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

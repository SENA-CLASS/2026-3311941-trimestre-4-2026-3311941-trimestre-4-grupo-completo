...
import { sampleWithRequiredData } from '../project-phase.test-samples';
...
describe('ProjectPhase Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'd6306830-4cd7-49d4-ae49-a650a81278f2' }], {
    ...
    expect(comp.projectPhases()[0]).toEqual(expect.objectContaining({ id: 'd6306830-4cd7-49d4-ae49-a650a81278f2' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'd6306830-4cd7-49d4-ae49-a650a81278f2' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.projectPhases()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'd6306830-4cd7-49d4-ae49-a650a81278f2' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to projectPhaseService', () => {
      ...
      vitest.spyOn(service, 'getProjectPhaseIdentifier');
      const id = comp.trackId(entity);
      expect(service.getProjectPhaseIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.projectPhasesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

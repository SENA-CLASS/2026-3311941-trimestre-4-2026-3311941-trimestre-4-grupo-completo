...
import boundingScheduleResolve from './bounding-schedule-routing-resolve.service';
...
describe('BoundingSchedule routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IBoundingSchedule returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          boundingScheduleResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(boundingScheduleResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');

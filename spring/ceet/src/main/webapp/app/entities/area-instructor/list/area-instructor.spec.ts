...
import { sampleWithRequiredData } from '../area-instructor.test-samples';
...
describe('AreaInstructor Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: AreaInstructor;
  let fixture: ComponentFixture<AreaInstructor>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(AreaInstructor);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'e0befa69-d67d-40bf-beb0-9493374d7174' }], {
    ...
    expect(comp.areaInstructors()[0]).toEqual(expect.objectContaining({ id: 'e0befa69-d67d-40bf-beb0-9493374d7174' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'e0befa69-d67d-40bf-beb0-9493374d7174' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.areaInstructors()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'e0befa69-d67d-40bf-beb0-9493374d7174' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to areaInstructorService', () => {
      const entity = { id: 'e0befa69-d67d-40bf-beb0-9493374d7174' };
      vitest.spyOn(service, 'getAreaInstructorIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAreaInstructorIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.areaInstructorsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

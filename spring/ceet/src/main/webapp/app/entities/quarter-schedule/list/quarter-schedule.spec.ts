...
import { sampleWithRequiredData } from '../quarter-schedule.test-samples';
...
describe('QuarterSchedule Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: QuarterSchedule;
  let fixture: ComponentFixture<QuarterSchedule>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(QuarterSchedule);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'cb8a58fb-ddc6-4c56-9542-9f8e60b8e090' }], {
    ...
    expect(comp.quarterSchedules()[0]).toEqual(expect.objectContaining({ id: 'cb8a58fb-ddc6-4c56-9542-9f8e60b8e090' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'cb8a58fb-ddc6-4c56-9542-9f8e60b8e090' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.quarterSchedules()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'cb8a58fb-ddc6-4c56-9542-9f8e60b8e090' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to quarterScheduleService', () => {
      ...
      vitest.spyOn(service, 'getQuarterScheduleIdentifier');
      const id = comp.trackId(entity);
      expect(service.getQuarterScheduleIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.quarterSchedulesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

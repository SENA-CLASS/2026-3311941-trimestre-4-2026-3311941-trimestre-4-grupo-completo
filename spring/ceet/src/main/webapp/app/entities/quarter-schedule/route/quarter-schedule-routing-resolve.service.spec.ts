...
import quarterScheduleResolve from './quarter-schedule-routing-resolve.service';

describe('QuarterSchedule routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IQuarterSchedule returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          quarterScheduleResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(quarterScheduleResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');

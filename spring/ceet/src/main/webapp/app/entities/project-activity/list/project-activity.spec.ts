...
import { sampleWithRequiredData } from '../project-activity.test-samples';
...
describe('ProjectActivity Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'ce18f414-2724-443b-a81a-1f9d591eb13c' }], {
    ...
    expect(comp.projectActivities()[0]).toEqual(expect.objectContaining({ id: 'ce18f414-2724-443b-a81a-1f9d591eb13c' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'ce18f414-2724-443b-a81a-1f9d591eb13c' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.projectActivities()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'ce18f414-2724-443b-a81a-1f9d591eb13c' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to projectActivityService', () => {
      ...
      vitest.spyOn(service, 'getProjectActivityIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.projectActivitiesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

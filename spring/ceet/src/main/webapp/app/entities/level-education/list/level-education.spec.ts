...
import { sampleWithRequiredData } from '../level-education.test-samples';
...
describe('LevelEducation Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: LevelEducation;
  let fixture: ComponentFixture<LevelEducation>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(LevelEducation);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '815b1471-c41c-4cfe-a7d8-56ff13b64510' }], {
    ...
    expect(comp.levelEducations()[0]).toEqual(expect.objectContaining({ id: '815b1471-c41c-4cfe-a7d8-56ff13b64510' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '815b1471-c41c-4cfe-a7d8-56ff13b64510' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.levelEducations()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '815b1471-c41c-4cfe-a7d8-56ff13b64510' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to levelEducationService', () => {
      ...
      vitest.spyOn(service, 'getLevelEducationIdentifier');
      const id = comp.trackId(entity);
      expect(service.getLevelEducationIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.levelEducationsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

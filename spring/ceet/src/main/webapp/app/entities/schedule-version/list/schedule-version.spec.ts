...
import { sampleWithRequiredData } from '../schedule-version.test-samples';
...
describe('ScheduleVersion Management Component', () => {
  ...
  let comp: ScheduleVersion;
  let fixture: ComponentFixture<ScheduleVersion>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(ScheduleVersion);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '92e0f35b-04d9-47d7-9946-bfa57a73f086' }], {
    ...
    expect(comp.scheduleVersions()[0]).toEqual(expect.objectContaining({ id: '92e0f35b-04d9-47d7-9946-bfa57a73f086' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '92e0f35b-04d9-47d7-9946-bfa57a73f086' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.scheduleVersions()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '92e0f35b-04d9-47d7-9946-bfa57a73f086' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to scheduleVersionService', () => {
      ...
      vitest.spyOn(service, 'getScheduleVersionIdentifier');
      const id = comp.trackId(entity);
      expect(service.getScheduleVersionIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.scheduleVersionsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

...
describe('DocumentType Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '4eea9f54-84fe-4142-b830-d89e8cb3c84a' }], {
    ...
    expect(comp.documentTypes()[0]).toEqual(expect.objectContaining({ id: '4eea9f54-84fe-4142-b830-d89e8cb3c84a' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '4eea9f54-84fe-4142-b830-d89e8cb3c84a' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.documentTypes()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '4eea9f54-84fe-4142-b830-d89e8cb3c84a' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to documentTypeService', () => {
      ...
      vitest.spyOn(service, 'getDocumentTypeIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.documentTypesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

...
import { sampleWithRequiredData } from '../viewed-result.test-samples';

import { ViewedResult } from './viewed-result';

vitest.useFakeTimers();

describe('ViewedResult Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: ViewedResult;
  let fixture: ComponentFixture<ViewedResult>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(ViewedResult);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'dceb3c65-7ad5-4d2c-adee-22240c1d2286' }], {
    ...
    expect(comp.viewedResults()[0]).toEqual(expect.objectContaining({ id: 'dceb3c65-7ad5-4d2c-adee-22240c1d2286' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'dceb3c65-7ad5-4d2c-adee-22240c1d2286' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.viewedResults()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'dceb3c65-7ad5-4d2c-adee-22240c1d2286' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to viewedResultService', () => {
      const entity = { id: 'dceb3c65-7ad5-4d2c-adee-22240c1d2286' };
      vitest.spyOn(service, 'getViewedResultIdentifier');
      const id = comp.trackId(entity);
      expect(service.getViewedResultIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.viewedResultsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

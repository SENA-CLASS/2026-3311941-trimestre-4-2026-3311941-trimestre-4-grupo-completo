...
public class TrimesterResource {
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<TrimesterDTO>> getAllTrimesters(
        @org.springdoc.core.annotations.ParameterObject Pageable pageable,
        @RequestParam(name = "eagerload", required = false, defaultValue = "true") boolean eagerload
    ) {
        LOG.debug("REST request to get a page of Trimesters");
        Page<TrimesterDTO> page;
        if (eagerload) {
            page = trimesterService.findAllWithEagerRelationships(pageable);
        } else {
            page = trimesterService.findAll(pageable);
        }

...
/**
 * Service Implementation for managing {@link co.edu.sena.domain.Trimester}.
 */
@Service
public class TrimesterServiceImpl implements TrimesterService {
    ...
    public Page<TrimesterDTO> findAllWithEagerRelationships(Pageable pageable) {
        return trimesterRepository.findAllWithEagerRelationships(pageable).map(trimesterMapper::toDto);
    }

    @Override
    public Optional<TrimesterDTO> findOne(String id) {
        LOG.debug("Request to get Trimester : {}", id);
        return trimesterRepository.findOneWithEagerRelationships(id).map(trimesterMapper::toDto);
    }

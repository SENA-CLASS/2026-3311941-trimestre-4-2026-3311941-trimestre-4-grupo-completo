...
import { sampleWithRequiredData } from '../training-program.test-samples';
...
describe('TrainingProgram Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'e35ca12a-dfa0-4f9c-9ffb-b83e56d621c3' }], {
    ...
    expect(comp.trainingPrograms()[0]).toEqual(expect.objectContaining({ id: 'e35ca12a-dfa0-4f9c-9ffb-b83e56d621c3' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'e35ca12a-dfa0-4f9c-9ffb-b83e56d621c3' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.trainingPrograms()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'e35ca12a-dfa0-4f9c-9ffb-b83e56d621c3' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to trainingProgramService', () => {
      ...
      vitest.spyOn(service, 'getTrainingProgramIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTrainingProgramIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.trainingProgramsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

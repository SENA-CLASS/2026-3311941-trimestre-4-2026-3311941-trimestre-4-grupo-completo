...
import { sampleWithRequiredData } from '../apprentice.test-samples';
...
describe('Apprentice Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: Apprentice;
  let fixture: ComponentFixture<Apprentice>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Apprentice);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '8681ca6b-c1b7-4e43-96b8-c3fc96b48213' }], {
    ...
    expect(comp.apprentices()[0]).toEqual(expect.objectContaining({ id: '8681ca6b-c1b7-4e43-96b8-c3fc96b48213' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '8681ca6b-c1b7-4e43-96b8-c3fc96b48213' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.apprentices()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '8681ca6b-c1b7-4e43-96b8-c3fc96b48213' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to apprenticeService', () => {
      ...
      vitest.spyOn(service, 'getApprenticeIdentifier');
      const id = comp.trackId(entity);
      expect(service.getApprenticeIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.apprenticesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

...
import { sampleWithRequiredData } from '../course-trimester.test-samples';
...
describe('CourseTrimester Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: CourseTrimester;
  let fixture: ComponentFixture<CourseTrimester>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CourseTrimester);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '87627c1c-a756-41da-8baf-2032937c03d7' }], {
    ...
    expect(comp.courseTrimesters()[0]).toEqual(expect.objectContaining({ id: '87627c1c-a756-41da-8baf-2032937c03d7' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '87627c1c-a756-41da-8baf-2032937c03d7' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.courseTrimesters()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '87627c1c-a756-41da-8baf-2032937c03d7' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to courseTrimesterService', () => {
      ...
      vitest.spyOn(service, 'getCourseTrimesterIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCourseTrimesterIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.courseTrimestersParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

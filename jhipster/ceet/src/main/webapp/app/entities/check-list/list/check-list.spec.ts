...
describe('CheckList Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '7741670b-56eb-400b-a3ea-2a7b30040404' }], {
    ...
    expect(comp.checkLists()[0]).toEqual(expect.objectContaining({ id: '7741670b-56eb-400b-a3ea-2a7b30040404' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '7741670b-56eb-400b-a3ea-2a7b30040404' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.checkLists()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '7741670b-56eb-400b-a3ea-2a7b30040404' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to checkListService', () => {
      ...
      vitest.spyOn(service, 'getCheckListIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.checkListsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

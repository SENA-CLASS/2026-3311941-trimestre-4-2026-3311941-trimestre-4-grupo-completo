...
import { sampleWithRequiredData } from '../course-status.test-samples';
...
describe('CourseStatus Management Component', () => {
  ...
  let comp: CourseStatus;
  let fixture: ComponentFixture<CourseStatus>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CourseStatus);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '10d644f6-f194-440a-bbab-b099a4c4d9fb' }], {
    ...
    expect(comp.courseStatuses()[0]).toEqual(expect.objectContaining({ id: '10d644f6-f194-440a-bbab-b099a4c4d9fb' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '10d644f6-f194-440a-bbab-b099a4c4d9fb' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.courseStatuses()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '10d644f6-f194-440a-bbab-b099a4c4d9fb' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to courseStatusService', () => {
      ...
      vitest.spyOn(service, 'getCourseStatusIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCourseStatusIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.courseStatusesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

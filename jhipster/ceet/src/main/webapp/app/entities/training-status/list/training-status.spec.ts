...
import { sampleWithRequiredData } from '../training-status.test-samples';
...
describe('TrainingStatus Management Component', () => {
  ...
  let comp: TrainingStatus;
  let fixture: ComponentFixture<TrainingStatus>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(TrainingStatus);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '053bf031-a284-46fd-8586-1a656a154cf3' }], {
    ...
    expect(comp.trainingStatuses()[0]).toEqual(expect.objectContaining({ id: '053bf031-a284-46fd-8586-1a656a154cf3' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '053bf031-a284-46fd-8586-1a656a154cf3' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.trainingStatuses()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '053bf031-a284-46fd-8586-1a656a154cf3' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to trainingStatusService', () => {
      ...
      vitest.spyOn(service, 'getTrainingStatusIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTrainingStatusIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.trainingStatusesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

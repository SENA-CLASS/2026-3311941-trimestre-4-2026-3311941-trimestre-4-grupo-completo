...
describe('Classroom Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'ef876767-91ce-4e28-b866-8ebdf1c26f6f' }], {
    ...
    expect(comp.classrooms()[0]).toEqual(expect.objectContaining({ id: 'ef876767-91ce-4e28-b866-8ebdf1c26f6f' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'ef876767-91ce-4e28-b866-8ebdf1c26f6f' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.classrooms()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'ef876767-91ce-4e28-b866-8ebdf1c26f6f' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to classroomService', () => {
      ...
      vitest.spyOn(service, 'getClassroomIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.classroomsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

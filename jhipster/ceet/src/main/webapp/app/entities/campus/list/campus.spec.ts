...
import { sampleWithRequiredData } from '../campus.test-samples';
...
describe('Campus Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'ef9c809c-3ee1-443a-82b3-b6747c4e5680' }], {
    ...
    expect(comp.campuses()[0]).toEqual(expect.objectContaining({ id: 'ef9c809c-3ee1-443a-82b3-b6747c4e5680' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'ef9c809c-3ee1-443a-82b3-b6747c4e5680' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.campuses()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'ef9c809c-3ee1-443a-82b3-b6747c4e5680' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to campusService', () => {
      ...
      vitest.spyOn(service, 'getCampusIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCampusIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.campusesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

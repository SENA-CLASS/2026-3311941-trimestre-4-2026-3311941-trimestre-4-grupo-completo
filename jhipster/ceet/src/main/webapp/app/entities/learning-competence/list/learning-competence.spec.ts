...
import { sampleWithRequiredData } from '../learning-competence.test-samples';
...
describe('LearningCompetence Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: LearningCompetence;
  let fixture: ComponentFixture<LearningCompetence>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(LearningCompetence);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '67a3ee40-7a88-4800-b512-8818586690b5' }], {
    ...
    expect(comp.learningCompetences()[0]).toEqual(expect.objectContaining({ id: '67a3ee40-7a88-4800-b512-8818586690b5' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '67a3ee40-7a88-4800-b512-8818586690b5' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.learningCompetences()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '67a3ee40-7a88-4800-b512-8818586690b5' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to learningCompetenceService', () => {
      ...
      vitest.spyOn(service, 'getLearningCompetenceIdentifier');
      const id = comp.trackId(entity);
      expect(service.getLearningCompetenceIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.learningCompetencesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

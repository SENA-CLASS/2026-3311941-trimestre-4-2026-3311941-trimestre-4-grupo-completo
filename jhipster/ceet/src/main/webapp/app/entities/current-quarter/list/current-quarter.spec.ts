...
import { sampleWithRequiredData } from '../current-quarter.test-samples';
...
describe('CurrentQuarter Management Component', () => {
  ...
  let comp: CurrentQuarter;
  let fixture: ComponentFixture<CurrentQuarter>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CurrentQuarter);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '0ed02114-2749-41f7-9a38-0f3348269a95' }], {
    ...
    expect(comp.currentQuarters()[0]).toEqual(expect.objectContaining({ id: '0ed02114-2749-41f7-9a38-0f3348269a95' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '0ed02114-2749-41f7-9a38-0f3348269a95' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.currentQuarters()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '0ed02114-2749-41f7-9a38-0f3348269a95' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to currentQuarterService', () => {
      ...
      vitest.spyOn(service, 'getCurrentQuarterIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCurrentQuarterIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.currentQuartersParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

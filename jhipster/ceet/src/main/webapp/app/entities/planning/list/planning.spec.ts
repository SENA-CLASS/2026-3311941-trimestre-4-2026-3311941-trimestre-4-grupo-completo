...
import { sampleWithRequiredData } from '../planning.test-samples';
...
describe('Planning Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '3c7d8900-e060-455b-8080-b0f040d98cdb' }], {
    ...
    expect(comp.plannings()[0]).toEqual(expect.objectContaining({ id: '3c7d8900-e060-455b-8080-b0f040d98cdb' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '3c7d8900-e060-455b-8080-b0f040d98cdb' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.plannings()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '3c7d8900-e060-455b-8080-b0f040d98cdb' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to planningService', () => {
      ...
      vitest.spyOn(service, 'getPlanningIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.planningsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

...
import { sampleWithRequiredData } from '../bonding.test-samples';
...
describe('Bonding Management Component', () => {
  ...
  let comp: Bonding;
  let fixture: ComponentFixture<Bonding>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Bonding);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '57960bcc-d6df-4acc-bce6-afeba60dd2e4' }], {
    ...
    expect(comp.bondings()[0]).toEqual(expect.objectContaining({ id: '57960bcc-d6df-4acc-bce6-afeba60dd2e4' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '57960bcc-d6df-4acc-bce6-afeba60dd2e4' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.bondings()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '57960bcc-d6df-4acc-bce6-afeba60dd2e4' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to bondingService', () => {
      ...
      vitest.spyOn(service, 'getBondingIdentifier');
      const id = comp.trackId(entity);
      expect(service.getBondingIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.bondingsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

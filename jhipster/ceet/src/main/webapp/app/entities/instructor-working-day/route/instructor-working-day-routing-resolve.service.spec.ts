...
import instructorWorkingDayResolve from './instructor-working-day-routing-resolve.service';

describe('InstructorWorkingDay routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IInstructorWorkingDay returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          instructorWorkingDayResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(instructorWorkingDayResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');

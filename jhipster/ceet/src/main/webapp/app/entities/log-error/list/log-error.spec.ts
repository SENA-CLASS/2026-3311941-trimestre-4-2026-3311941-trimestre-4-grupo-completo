...
import { sampleWithRequiredData } from '../log-error.test-samples';
...
describe('LogError Management Component', () => {
  ...
  let comp: LogError;
  let fixture: ComponentFixture<LogError>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(LogError);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '86ed6f82-2f50-4460-81da-2d13f2ad36ef' }], {
    ...
    expect(comp.logErrors()[0]).toEqual(expect.objectContaining({ id: '86ed6f82-2f50-4460-81da-2d13f2ad36ef' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '86ed6f82-2f50-4460-81da-2d13f2ad36ef' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.logErrors()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '86ed6f82-2f50-4460-81da-2d13f2ad36ef' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to logErrorService', () => {
      const entity = { id: '86ed6f82-2f50-4460-81da-2d13f2ad36ef' };
      vitest.spyOn(service, 'getLogErrorIdentifier');
      const id = comp.trackId(entity);
      expect(service.getLogErrorIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.logErrorsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

...
import { sampleWithRequiredData } from '../item-list.test-samples';
...
describe('ItemList Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'f5c01f94-408b-48fb-9301-893ca4de0dc0' }], {
    ...
    expect(comp.itemLists()[0]).toEqual(expect.objectContaining({ id: 'f5c01f94-408b-48fb-9301-893ca4de0dc0' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'f5c01f94-408b-48fb-9301-893ca4de0dc0' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.itemLists()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'f5c01f94-408b-48fb-9301-893ca4de0dc0' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to itemListService', () => {
      ...
      vitest.spyOn(service, 'getItemListIdentifier');
      const id = comp.trackId(entity);
      expect(service.getItemListIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.itemListsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

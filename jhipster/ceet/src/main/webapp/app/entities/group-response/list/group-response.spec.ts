...
import { sampleWithRequiredData } from '../group-response.test-samples';
...
describe('GroupResponse Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: GroupResponse;
  let fixture: ComponentFixture<GroupResponse>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(GroupResponse);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '57b45d23-226e-48e7-93c6-046bd207c9c6' }], {
    ...
    expect(comp.groupResponses()[0]).toEqual(expect.objectContaining({ id: '57b45d23-226e-48e7-93c6-046bd207c9c6' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '57b45d23-226e-48e7-93c6-046bd207c9c6' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.groupResponses()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '57b45d23-226e-48e7-93c6-046bd207c9c6' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to groupResponseService', () => {
      ...
      vitest.spyOn(service, 'getGroupResponseIdentifier');
      const id = comp.trackId(entity);
      expect(service.getGroupResponseIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.groupResponsesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

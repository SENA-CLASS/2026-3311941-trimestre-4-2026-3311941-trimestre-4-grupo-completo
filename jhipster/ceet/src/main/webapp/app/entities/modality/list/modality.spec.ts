...
import { sampleWithRequiredData } from '../modality.test-samples';
...
describe('Modality Management Component', () => {
  ...
  let comp: Modality;
  let fixture: ComponentFixture<Modality>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Modality);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'd35eac1f-455b-4c70-9249-9dc25997a012' }], {
    ...
    expect(comp.modalities()[0]).toEqual(expect.objectContaining({ id: 'd35eac1f-455b-4c70-9249-9dc25997a012' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'd35eac1f-455b-4c70-9249-9dc25997a012' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.modalities()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'd35eac1f-455b-4c70-9249-9dc25997a012' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to modalityService', () => {
      ...
      vitest.spyOn(service, 'getModalityIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.modalitiesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

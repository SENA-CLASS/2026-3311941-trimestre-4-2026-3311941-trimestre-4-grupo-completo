...
import { sampleWithRequiredData } from '../bounding-schedule.test-samples';
...
describe('BoundingSchedule Management Component', () => {
  ...
  let comp: BoundingSchedule;
  let fixture: ComponentFixture<BoundingSchedule>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(BoundingSchedule);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '25fe1b48-f026-4b16-aaf7-f684c5be822a' }], {
    ...
    expect(comp.boundingSchedules()[0]).toEqual(expect.objectContaining({ id: '25fe1b48-f026-4b16-aaf7-f684c5be822a' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '25fe1b48-f026-4b16-aaf7-f684c5be822a' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.boundingSchedules()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '25fe1b48-f026-4b16-aaf7-f684c5be822a' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to boundingScheduleService', () => {
      const entity = { id: '25fe1b48-f026-4b16-aaf7-f684c5be822a' };
      vitest.spyOn(service, 'getBoundingScheduleIdentifier');
      const id = comp.trackId(entity);
      expect(service.getBoundingScheduleIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.boundingSchedulesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

...
import { sampleWithRequiredData } from '../planning-activity.test-samples';
...
describe('PlanningActivity Management Component', () => {
  ...
  let comp: PlanningActivity;
  let fixture: ComponentFixture<PlanningActivity>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(PlanningActivity);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 'e33c8a7c-2d1d-41a5-b595-8ae92ad792d5' }], {
    ...
    expect(comp.planningActivities()[0]).toEqual(expect.objectContaining({ id: 'e33c8a7c-2d1d-41a5-b595-8ae92ad792d5' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 'e33c8a7c-2d1d-41a5-b595-8ae92ad792d5' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.planningActivities()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 'e33c8a7c-2d1d-41a5-b595-8ae92ad792d5' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to planningActivityService', () => {
      const entity = { id: 'e33c8a7c-2d1d-41a5-b595-8ae92ad792d5' };
      vitest.spyOn(service, 'getPlanningActivityIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPlanningActivityIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.planningActivitiesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

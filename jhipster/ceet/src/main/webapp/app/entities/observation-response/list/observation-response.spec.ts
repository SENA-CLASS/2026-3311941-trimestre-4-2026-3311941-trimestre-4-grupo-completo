...
import { sampleWithRequiredData } from '../observation-response.test-samples';
...
describe('ObservationResponse Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: ObservationResponse;
  let fixture: ComponentFixture<ObservationResponse>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(ObservationResponse);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: '4d4f5219-1b15-4afc-b043-9d7c859160bc' }], {
    ...
    expect(comp.observationResponses()[0]).toEqual(expect.objectContaining({ id: '4d4f5219-1b15-4afc-b043-9d7c859160bc' }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: '4d4f5219-1b15-4afc-b043-9d7c859160bc' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.observationResponses()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: '4d4f5219-1b15-4afc-b043-9d7c859160bc' }], {
      headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' },
    });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to observationResponseService', () => {
      const entity = { id: '4d4f5219-1b15-4afc-b043-9d7c859160bc' };
      vitest.spyOn(service, 'getObservationResponseIdentifier');
      const id = comp.trackId(entity);
      expect(service.getObservationResponseIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.observationResponsesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });

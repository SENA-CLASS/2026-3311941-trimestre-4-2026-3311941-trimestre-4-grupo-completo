...
public class TrainingProgramResource {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(TrainingProgramResource.class);
    ...
    public ResponseEntity<TrainingProgramDTO> createTrainingProgram(@Valid @RequestBody TrainingProgramDTO trainingProgramDTO)
        ...
        return ResponseEntity.created(new URI("/api/training-programs/" + trainingProgramDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, trainingProgramDTO.getId()))
    ...
    public ResponseEntity<TrainingProgramDTO> updateTrainingProgram(
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, trainingProgramDTO.getId()))
            .body(trainingProgramDTO);
    }
    ...
    public ResponseEntity<TrainingProgramDTO> partialUpdateTrainingProgram(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, trainingProgramDTO.getId())
        );
    }

    /**
     * {@code GET  /training-programs} : get all the Training Programs.
     *
     * @param pageable the pagination information.
     * @param eagerload flag to eager load entities from relationships (This is applicable for many-to-many).
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of Training Programs in body.
     */
    @GetMapping("")
    public ResponseEntity<List<TrainingProgramDTO>> getAllTrainingPrograms(
        ...
        Page<TrainingProgramDTO> page;
        if (eagerload) {
            page = trainingProgramService.findAllWithEagerRelationships(pageable);
        } else {
            page = trainingProgramService.findAll(pageable);
        }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<TrainingProgramDTO> getTrainingProgram(@PathVariable("id") String id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteTrainingProgram(@PathVariable("id") String id) {
